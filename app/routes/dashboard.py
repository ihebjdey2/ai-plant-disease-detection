...
def index():
    ...
    return render_template("index.html", total_scans=total, healthy_scans=counts.get("healthy",0), diseased_scans=counts.get("diseased",0), uncertain_scans=counts.get("uncertain",0), no_leaf_scans=counts.get("no_leaf",0), average_confidence=q.with_entities(func.avg(Prediction.confidence)).scalar() or 0, recent_predictions=q.order_by(Prediction.created_at.desc()).limit(5).all(), frequent_diseases=frequent, supported_crops=supported_crops())
