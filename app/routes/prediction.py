...
def create():
    ...
    if not file or not file.filename: flash(translate("Choose an image to analyze."),"error"); return redirect(url_for("dashboard.index"))
    ext=file.filename.rsplit(".",1)[-1].lower() if "." in file.filename else ""
    if ext not in current_app.config["ALLOWED_IMAGE_EXTENSIONS"]: flash(translate("Supported formats: JPG, PNG, WEBP."),"error"); return redirect(url_for("dashboard.index"))
    ...
    try: result=predict(path)
        ...
        if get_locale() != "en" and message == str(exc):
            message = translate("The image could not be analyzed.")
        flash(message, "error")
